...
function IsPlayerLogin() {
    ...
    return (API.hasEntitySyncedData(player, "PLAYER_ISLOGIN"));
}

API.onUpdate.connect(function () {
    // MultiplayerInfo
    if (!API.isChatOpen() && IsPlayerLogin() && API.isControlJustPressed(20 /* MultiplayerInfo */) ) {
        g_currentState++;
    ...
    if (g_currentState == 1) {
        ...
        // Fill
        API.drawRectangle(listX, listY, listWidth, listHeight, 102, 102, 153, 220);
        // Separator
        API.drawRectangle(listX, listY + listLine, listWidth, listBorder, 255, 255, 255, 220);
        // Left
        ...
        // Header
        API.drawText("Имя", listX + listPadding, listY - listPadding / 2, 0.4, 255, 255, 255, 255, 0, 0, true, true, 0);
        API.drawText("Пинг", listX + listWidth - listPadding, listY - listPadding / 2, 0.4, 255, 255, 255, 255, 0, 2, true, true, 0);
        // Players
        for (var i = 0; i < listPageCount; i++) {
            ...
            API.drawText("" + player.ping, listX + listWidth - listPadding, listY + listPadding / 2 + listLine * (i + 1), 0.45, 255, 0, 0, 255, 4, 2, true, true, 0);
        }
    }
});
